...
-- FIXED DRAGGABLE FUNCTION FOR MOBILE (TOUCH COMPATIBLE)
local function draggable(obj)
	...
	local function update(input)
		...
		local Tween = TweenService:Create(gui, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = EndPos})
		Tween:Play()
	end
...
function lib:Window(text)
	...
	VenLib.ResetOnSpawn = false -- Crucial: Prevents UI from resetting/vanishing when resetting
	...
	function tabs:Tab(title)
		...
		-- FIXED SLIDER FUNCTION (ADDED TOUCH INPUT INTERACTION)
		function tab:Slider(text, min, max, start, callback)
			...
			local function slide(input)
				local pos = UDim2.new(math.clamp((input.Position.X - placetoslide.AbsolutePosition.X) / placetoslide.AbsoluteSize.X, 0, 1), 0, 1, 0)
